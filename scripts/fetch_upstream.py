...
def provenance_block(name, entry):
    ...
    lines += [
        '',
        'The body above is the upstream text at that commit, unchanged. The only edit',
        f"is the frontmatter `name:`, rewritten to `{name}` so the skill installs under the",
        'forge naming scheme; every other frontmatter field is upstream\'s.',
        '',
...
def cmd_fetch(args, lock):
    names = select(lock, args.skills, args.non_interactive)
...
def cmd_verify(args, lock):
    ...
    if bad:
        print(f"\n  {bad} skill(s) need a re-fetch. Run: python3 scripts/fetch_upstream.py")
